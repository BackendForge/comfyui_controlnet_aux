...
def common(name, processor, img):
    ...
    output(
        name + "_np_np",
        Image.fromarray(processor(np.array(img, dtype=np.uint8), output_type="np")),
    )
    output(
        name + "_np_pil", processor(np.array(img, dtype=np.uint8), output_type="pil")
    )
    output(
        name + "_scaled", processor(img, detect_resolution=640, image_resolution=768)
    )
...
def test_openpose(img):
    ...
    output(
        "openpose_face",
        openpose(img, include_body=True, include_hand=False, include_face=True),
    )
    output(
        "openpose_faceonly",
        openpose(img, include_body=False, include_hand=False, include_face=True),
    )
    output(
        "openpose_full",
        openpose(img, include_body=True, include_hand=True, include_face=True),
    )
    output(
        "openpose_hand",
        openpose(img, include_body=True, include_hand=True, include_face=False),
    )
